...
import { useTrailerById } from "@/features/trailers/hooks";
...
export default function TrailerVideoPage() {
  const params = useParams<{ id: string }>();
  const trailerId = params.id;
  const fileInputRef = useRef<HTMLInputElement>(null);

  const trailerQuery = useTrailerById(trailerId);
  const videoQuery = useVideoByOwner("trailer", trailerId);
  ...
  const isCurrentUpload =
    progress.ownerType === "trailer" && progress.ownerId === trailerId;
  ...
  function handleFileSelect(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (!file || !trailerQuery.data) return;
    startUpload("trailer", trailerId, trailerQuery.data.title, file);
    e.target.value = "";
  }
        ...
        <div className="flex items-center gap-2 px-4">
          ...
          <Breadcrumb>
            <BreadcrumbList>
              ...
              <BreadcrumbItem>
                <BreadcrumbPage>
                  {trailerName ? `Video: ${trailerName}` : "Video"}
                </BreadcrumbPage>
              </BreadcrumbItem>
            </BreadcrumbList>
          </Breadcrumb>
      ...
      <div className="flex flex-1 flex-col gap-6 p-4 pt-0">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Upload and manage the video file for this trailer.
          </p>
        </div>
